...
export default function StockDonePlates() {
    ...
    let noteCode = useSelector(store => store.codeReducer.noteCode)
    let filterDoneCode = useSelector(store => store.codeReducer.filterDoneCode)
    console.log("🚀 ~ file: Stock-3-done.jsx ~ line 75 ~ StockDonePlates ~ filterDoneCode", filterDoneCode)
    ...
    const editFields = (item, itemStock, id, listSinks, comments) => {
        ...
        let list = []
        for (let i = 0; i < filterDoneCode.length; i++) {
            list.push({ listaClases: [] })
            filterDoneCode[i].stock.map(() => list[i].listaClases.push({ clase: "" }))
        }
                        ...
                        <div className='boxStockCard-containerSink'>
                            {
                                ...
                                (
                                    stock.plate ?
                                        (<div className='boxStockCard-sink'>
                                            ...
                                            <div className='boxStockCard-data'>
                                                ...
                                                <div className='boxStockCard-botones'>
                                                    ...
                                                    {code.stock.map((e, i) => e.sink ?
                                                        <button key={i} onClick={() => verPlacas(index)} className='btnEditar' type='button' >{clasePlaca[index]?.clase ? 'Ocultar piletas' : "Ver piletas"} </button>
                                                        : null)}
                                                </div>
                                            </div>
                                        </div>)
                                        ...
                                        : stock.sink ?
                                            (<div key={stock._id} className={clasePlaca[index]?.clase ? ' boxStockCard-sink' : 'displeyNone boxStockCard-sink'} >
