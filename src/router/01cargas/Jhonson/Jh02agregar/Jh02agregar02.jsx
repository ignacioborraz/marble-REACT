...
export default function Jh02agregar01() {
    ...
    useEffect(() => {
        ...
        axios.get(`${apiUrl}clients`,headers).then(res => setCurrentClient(res.data.response.clients.find(each=>each._id===id_client)))
        ...
        if (ksink) {
            ...
            try {
                ...
                if (response.data.response===true) {
                    ...
                    let navigation = {
                        isConfirmed: `/add-plate/${id_code}/${id_client}`,
                        isDenied: `/add-jhonson/${id_code}/${id_client}`,
                        else: "/index"
                    }
        ...
        <div className='jhonson-container'>
            <div className='jhonson-middle jh-form'>
                ...
                <form className='jhonson-jhonson'>
                    <span className="jhonson-note jhonson-size">
                        CLIENTE: {(currentClient?.name&&currentClient?.company) ? (currentClient.name+' - '+currentClient.company) : (currentClient.name||currentClient.company)}
                    </span>
